...
;; Screenshot
(defun dehong//insert-org-or-md-img-link (prefix imagename)
  (if (equal (file-name-extension (buffer-file-name)) "org")
      (insert (format "[[%s%s]]" prefix imagename))
    (insert (format "![%s](%s%s)" imagename prefix imagename))))

(defun dehong/capture-screenshot (basename)
  ...
  (if (file-exists-p (file-name-directory fullpath))
      (progn
        ...
        (dehong//insert-org-or-md-img-link "../img/" relativepath))
    (progn
      (call-process "screencapture" nil nil nil "-s" (concat basename ".png"))
      (dehong//insert-org-or-md-img-link "./" (concat basename ".png"))))
  (insert "\n"))

(defun dehong/org-archive-done-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun dehong/org-archive-cancel-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun dehong/skip-non-stuck-projects ()
...
(defun dehong/org-insert-src-block (src-code-type)
...
(defun dehong/org-summary-todo (n-done n-not-done)
  "Switch entry to DONE when all subentries are done, to TODO otherwise."
  (let (org-log-done org-log-states)    ; turn off logging
    (org-todo (if (= n-not-done 0) "DONE" "TODO"))))

(defun dehong/filter-by-tags ()
  (let ((head-tags (org-get-tags-at)))
    (member current-tag head-tags)))

(defun dehong/org-clock-sum-today-by-tags (timerange &optional tstart tend noinsert)
  (interactive "P")
    ...
    (while (setq file (pop files))
      ...
      (with-current-buffer org-agenda-buffer
        (dolist (current-tag include-tags)
          (org-clock-sum tstart tend 'dehong/filter-by-tags)
